...
try:
    ...
    pass  # En Render no hay .env; usar variables de entorno
import secrets
...
# Usuario admin de cartera: ve todos los clientes (Isidro José)
CARTERA_ADMIN_USER_ID = 60

def is_cartera_admin(user):
    """True si el usuario es admin de cartera y ve todos los clientes."""
    return user and user.get("id") == CARTERA_ADMIN_USER_ID

# URL de la base de datos (local Docker o Render)
_raw = (os.getenv("DATABASE_URL") or "").strip()
if not _raw or _raw.lower() in ("null", "none", "undefined"):
    DATABASE_URL = "postgresql://jdm:jdm@localhost:5433/jdmcash"
else:
    DATABASE_URL = _raw
# Render usa postgres:// pero psycopg2 requiere postgresql://
if DATABASE_URL.startswith("postgres://"):
    DATABASE_URL = "postgresql://" + DATABASE_URL[11:]
USE_SSL = "sslmode=require" in DATABASE_URL or ("localhost" not in DATABASE_URL and "127.0.0.1" not in DATABASE_URL)
...
def get_conn():
    ...
    return psycopg2.connect(DATABASE_URL, **opts)

# ============================================================
# 🔄 GENERAR CUOTAS ATRASADAS AUTOMÁTICO (CORREGIDO)
# ============================================================

def generar_atrasos(org_id=None):
    """Procesa cuotas atrasadas. Si org_id se proporciona, solo préstamos de esa org."""
    ...
    org_filter = " AND organization_id = %s" if org_id is not None else ""
...
# Migraciones se ejecutan en _startup() después de init_db
...
def init_db():
    ...
    try:
        ...
        # ✅ Asegurar columna ruta en clients
        cur.execute("""
            ALTER TABLE clients
        ...
        # ---- Tabla organizations (empresa única / compatibilidad columnas organization_id) ----
        cur.execute("""
        ...
        """)
        cur.execute("ALTER TABLE organizations ADD COLUMN IF NOT EXISTS plan_id INTEGER DEFAULT 1;")
        cur.execute("ALTER TABLE organizations ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true;")
        cur.execute("ALTER TABLE organizations ADD COLUMN IF NOT EXISTS expires_at DATE;")
        ...
        # Quitar rol antiguo super_admin (panel multi-empresa eliminado)
        cur.execute("UPDATE users SET role = 'admin' WHERE role = 'super_admin';")
...
def calc_client_score(conn, client_id, org_id):
    """Score 0–100 según préstamos cerrados y atrasos impagos."""
    cur = conn.cursor()
    try:
        cur.execute(
            """
            ...
            WHERE client_id = %s AND organization_id = %s
              AND UPPER(COALESCE(TRIM(status), '')) IN ('CERRADO', 'PAGADO', 'FINALIZADO')
            """,
            (client_id, org_id),
        )
        prestamos_pagados = int((cur.fetchone() or {}).get("c") or 0)
        cur.execute(
            """
            SELECT COUNT(*) AS c
            FROM loan_arrears la
            JOIN loans l ON l.id = la.loan_id
            WHERE l.client_id = %s AND l.organization_id = %s AND la.paid = false
            """,
            (client_id, org_id),
        )
        atrasos = int((cur.fetchone() or {}).get("c") or 0)
    finally:
        cur.close()
    base = 55
...
def calc_max_credito(prestamos_pagados, score):
    """Monto orientativo recomendado (RD$)."""
...
def evaluate_loan(score, prestamos_activos):
    """approved / warning / denied para el formulario de nuevo préstamo."""
    if prestamos_activos >= 3:
        return {
            "status": "warning",
            "status_label": "Varios préstamos activos — revisar capacidad de pago.",
            "max_amount": round(max(3000, score * 80), -2),
        }
    if score < 40:
        return {
            "status": "denied",
            "status_label": "No recomendado — score bajo o muchos atrasos.",
            "max_amount": round(max(1000, score * 50), -2),
        }
    if score < 60:
        return {
            "status": "warning",
            "status_label": "Aprobado con reservas — monto limitado.",
            "max_amount": round(max(5000, score * 120), -2),
        }
    return {
        "status": "approved",
        "status_label": "Aprobado — buen historial.",
        "max_amount": round(max(8000, score * 200), -2),
    }
...
@media (max-width: 768px) {
  .side-menu a, .kpi-link, .btn, button, a.btn {
    min-height: 44px;
    min-width: 44px;
    display: inline-flex;
    align-items: center;
  ...
  .container { padding: 12px; padding-top: env(safe-area-inset-top, 12px); }
}

/* Pull to refresh indicator */
.ptr-indicator {
  position: fixed; top: 0; left: 0; right: 0;
  height: 50px; background: #16a34a; color: white;
...
<div id="ptrIndicator" class="ptr-indicator">↻ Actualizando...</div>
...
(function(){
  ...
  document.addEventListener("touchend", function(e){
    const ptrEnd = e.changedTouches[0].clientY;
    if (ptrStart > 0 && ptrEnd - ptrStart > 80 && window.scrollY < 10) {
      document.getElementById("ptrIndicator").classList.add("show");
...
// 🔔 Notificaciones: permiso + recordatorios (morosos, cobros hoy) cada 10 min
(function(){
  ...
  setInterval(function(){
    ...
    fetch("/api/notification-check", {credentials: "same-origin"})
      .then(r => r.json())
      .then(function(d){
        if (d.show && document.visibilityState !== "visible") {
          new Notification(d.title || "JDM Cash Now", {body: d.body});
        }
      })
      .catch(function(){});
  }, 600000);
...
{% if user and user.role == 'cobrador' %}
<script>
// Enviar ubicación cada 45 segundos cuando el cobrador tiene la app abierta
(function(){
  ...
  function sendGps() {
    navigator.geolocation.getCurrentPosition(
      function(pos) {
        ...
        fetch('/gps/update', { method: 'POST', body: fd });
      },
      function() {},
...
# ensure_users_phone_column se ejecuta en schema_migrations
...
def login():
    try:
        ...
        flash("Error del servidor. Intente de nuevo o contacte al administrador.", "danger")
...
def _login_impl():
    if request.method == "POST":
        ...
        session["org_id"] = user.get("organization_id") or user.get("org_id")
        session["collector_name"] = user.get("name")
        session["collector_phone"] = user.get("phone")

        try:
            log_action(user["id"], "login", "Inicio de sesion")
        except Exception:
...
def manifest():
    ...
    return jsonify({
        "name": "JDM Cash Now",
        "short_name": "JDM Cash",
        "description": "Sistema de préstamos: cobros, rutas y banco.",
        "start_url": "/dashboard",
        "scope": "/",
        "display": "standalone",
...
def api_notification_check():
    """API para recordatorios: morosos y cobros próximos (notificaciones locales)"""
    from flask import jsonify
    user = current_user()
    ...
    user_filter = "AND l.created_by = %s" if user.get("role") == "cobrador" and not is_cartera_admin(user) else ""
    params = [org_id]
    if user_filter:
        params.append(user["id"])

    cur.execute(f"""
        SELECT COUNT(*) AS c FROM loan_arrears la
        JOIN loans l ON l.id = la.loan_id
        WHERE la.paid = false AND l.organization_id = %s {user_filter}
    """, params)
    morosos = int((cur.fetchone() or {}).get("c") or 0)

    cur.execute(f"""
        SELECT COUNT(*) AS c FROM loans l
        WHERE l.organization_id = %s AND UPPER(l.status) = 'ACTIVO'
        AND l.next_payment_date = CURRENT_DATE {user_filter}
    """, params)
    cobros_hoy = int((cur.fetchone() or {}).get("c") or 0)

    cur.close()
    conn.close()

    if morosos > 0:
        return jsonify({"show": True, "title": "⚠️ Clientes morosos", "body": f"{morosos} cliente(s) con cuotas atrasadas"})
    if cobros_hoy > 0:
        return jsonify({"show": True, "title": "💰 Cobros hoy", "body": f"{cobros_hoy} cliente(s) para cobrar hoy"})
    return jsonify({"show": False})
...
def dashboard():
    ...
    org_id = session.get("org_id") or user.get("organization_id") or user.get("org_id")

    # Si no tiene org_id: intentar asignar/crear org Principal (evita logout loop)
    if not org_id:
        oid = None
        conn_aux = cur_aux = None
        try:
            conn_aux = get_conn()
            cur_aux = conn_aux.cursor()
            cur_aux.execute("SELECT id FROM organizations WHERE slug = 'principal' OR id = 1 LIMIT 1")
            row = cur_aux.fetchone()
            if row:
                oid = row.get("id") if hasattr(row, "get") else (row[0] if row else None)
            else:
                try:
                    cur_aux.execute("""
                        INSERT INTO organizations (name, slug, plan_id, is_active, expires_at)
                        VALUES ('Principal', 'principal', 1, true, '2030-12-31')
                        RETURNING id
                    """)
                    r = cur_aux.fetchone()
                    oid = r.get("id") if r and hasattr(r, "get") else (r[0] if r else None)
                except Exception:
                    if conn_aux:
                        conn_aux.rollback()
                    cur_aux.execute("SELECT id FROM organizations WHERE slug = 'principal' LIMIT 1")
                    r = cur_aux.fetchone()
                    oid = r.get("id") if r and hasattr(r, "get") else (r[0] if r else None)
            if oid:
                cur_aux.execute("UPDATE users SET organization_id = %s WHERE id = %s", (oid, user["id"]))
                conn_aux.commit()
                session["org_id"] = oid
                org_id = oid
        except Exception as e:
            print("Dashboard org_id fix:", str(e))
            if conn_aux:
                try:
                    conn_aux.rollback()
                except Exception:
            ...
            if conn_aux:
                try:
                    ...
                    pass
        if not org_id:
            flash("Sin organizacion asignada. Contacte al administrador.", "danger")
            return redirect(url_for("logout"))

    conn = get_conn()
    cur = conn.cursor()

    org_filter = f" AND organization_id = {org_id}"
    user_filter_sql = ""
    pay_filter_sql = ""
    total_clients = active_loans = capital_prestado = total_en_calle = interes_pagado = 0
    kpi_semanal = kpi_mensual = kpi_anual = cobrado_hoy = interes_hoy = prestamos_atrasados = 0
    total_empleados = 0
    # Métricas del Resumen Financiero
    cap_prestado = cap_cobrado = cap_pendiente = 0.0
    int_total = int_cobrado = int_pendiente = total_por_cobrar = 0.0

    if user["role"] == "cobrador" and not is_cartera_admin(user):
        user_filter_sql = f" AND created_by = {user['id']}"
        pay_filter_sql = f" AND p.created_by = {user['id']}"
        loan_filter_fin = f" AND created_by = {user['id']}"
    else:
        user_filter_sql = ""
        pay_filter_sql = ""
        loan_filter_fin = ""
    ...
    try:
        # ========== RESUMEN FINANCIERO (loans = prestamos, payments = pagos) ==========
        cur.execute(f"""
            SELECT COALESCE(SUM(amount), 0) AS capital_prestado
            FROM loans
            WHERE organization_id = {org_id} {loan_filter_fin}
        """)
        cap_prestado = float((cur.fetchone() or {}).get("capital_prestado") or 0)

        cur.execute(f"""
            SELECT COALESCE(SUM(p.capital), 0) AS capital_cobrado
            FROM payments p
            JOIN loans l ON l.id = p.loan_id
            WHERE l.organization_id = {org_id}
            AND COALESCE(p.status, 'OK') <> 'ANULADO' {pay_filter_sql}
        """)
        cap_cobrado = float((cur.fetchone() or {}).get("capital_cobrado") or 0)

        cap_pendiente = max(cap_prestado - cap_cobrado, 0)

        cur.execute(f"""
            SELECT COALESCE(SUM(amount * rate / 100 * COALESCE(NULLIF(term_count, 0), 1)), 0) AS interes_total
            FROM loans
            WHERE organization_id = {org_id} {loan_filter_fin}
        """)
        int_total = float((cur.fetchone() or {}).get("interes_total") or 0)

        cur.execute(f"""
            SELECT COALESCE(SUM(p.interest), 0) AS interes_cobrado
            FROM payments p
            JOIN loans l ON l.id = p.loan_id
            WHERE l.organization_id = {org_id}
            AND COALESCE(p.status, 'OK') <> 'ANULADO' {pay_filter_sql}
        """)
        int_cobrado = float((cur.fetchone() or {}).get("interes_cobrado") or 0)

        int_pendiente = max(int_total - int_cobrado, 0)
        total_por_cobrar = cap_pendiente + int_pendiente

        query = f"""
        SELECT
            (SELECT COUNT(*) FROM clients WHERE 1=1 {org_filter}{user_filter_sql}) AS total_clients,

            (SELECT COUNT(*) FROM loans WHERE UPPER(status)='ACTIVO' {org_filter}{user_filter_sql}) AS active_loans,

            (
                SELECT COALESCE(SUM(l.remaining),0)
                FROM loans l
                WHERE UPPER(l.status)='ACTIVO' {org_filter}{user_filter_sql}
            ) AS capital,

            (
                SELECT COALESCE(SUM(remaining),0)
                FROM loans
                WHERE UPPER(status)='ACTIVO' {org_filter}{user_filter_sql}
            ) AS total_en_calle,
            ...
            (
                ...
                FROM payments p
                JOIN loans l ON l.id = p.loan_id
                WHERE p.status <> 'ANULADO' AND l.organization_id = {org_id}
                {pay_filter_sql}
            ) AS interes_pagado,
            (
                SELECT COALESCE(SUM(p.amount),0)
                FROM payments p JOIN loans l ON l.id = p.loan_id
                WHERE DATE(p.date)=CURRENT_DATE AND COALESCE(p.status,'OK') <> 'ANULADO'
                AND l.organization_id = {org_id} {pay_filter_sql}
            ) AS cobrado_hoy,

            (
                SELECT COALESCE(SUM(p.interest),0)
                FROM payments p JOIN loans l ON l.id = p.loan_id
                WHERE DATE(p.date)=CURRENT_DATE AND COALESCE(p.status,'OK') <> 'ANULADO'
                AND l.organization_id = {org_id} {pay_filter_sql}
            ) AS interes_hoy,
            ...
            (
                ...
                WHERE UPPER(status)='ATRASADO' {org_filter}{user_filter_sql}
            ) AS prestamos_atrasados,

            (
                SELECT COALESCE(SUM(p.amount),0)
                FROM payments p JOIN loans l ON l.id = p.loan_id
                WHERE p.date >= CURRENT_DATE - INTERVAL '7 days' AND l.organization_id = {org_id} {pay_filter_sql}
            ) AS kpi_semanal,

            (
                SELECT COALESCE(SUM(p.amount),0)
                FROM payments p JOIN loans l ON l.id = p.loan_id
                WHERE COALESCE(p.status,'OK') <> 'ANULADO'
            ...
            (
                ...
                FROM payments p JOIN loans l ON l.id = p.loan_id
                WHERE COALESCE(p.status,'OK') <> 'ANULADO'
                AND DATE_TRUNC('year', p.date)=DATE_TRUNC('year', CURRENT_DATE)
                AND l.organization_id = {org_id} {pay_filter_sql}
            ) AS kpi_anual
        """
        ...
        active_loans = row.get("active_loans") or 0
        capital_prestado = float(row.get("capital") or 0)
        total_en_calle = float(row.get("total_en_calle") or 0)
        interes_pagado = float(row.get("interes_pagado") or 0)
        kpi_semanal = float(row.get("kpi_semanal") or 0)
        kpi_mensual = float(row.get("kpi_mensual") or 0)
        kpi_anual = float(row.get("kpi_anual") or 0)

        cobrado_hoy = float(row.get("cobrado_hoy") or 0)
        interes_hoy = float(row.get("interes_hoy") or 0)
        prestamos_atrasados = row.get("prestamos_atrasados") or 0

        total_empleados = 0
        if user["role"] in ("admin", "supervisor"):
            cur.execute("SELECT COUNT(*) AS total FROM users WHERE role='cobrador' AND organization_id = %s", (org_id,))
            te_row = cur.fetchone()
            total_empleados = int((te_row or {}).get("total", 0) if hasattr(te_row or {}, "get") else (te_row[0] if te_row else 0))

    except Exception as e:
        print("Dashboard query error:", str(e))
...
body.theme-dark .dash-section-title {{
  color:#e5e7eb;
}}
...
<div class="dash-section-title">📊 Resumen Financiero</div>
<div class="kpi-grid">

<div class="kpi-card" style="background:linear-gradient(135deg,#0f766e,#14b8a6)">
<div class="kpi-icon">💰</div>
<div class="kpi-title">Capital prestado</div>
<div class="value">{fmt_money(cap_prestado)}</div>
</div>

<div class="kpi-card" style="background:linear-gradient(135deg,#16a34a,#22c55e)">
<div class="kpi-icon">💵</div>
<div class="kpi-title">Capital cobrado</div>
<div class="value">{fmt_money(cap_cobrado)}</div>
</div>

<div class="kpi-card" style="background:linear-gradient(135deg,#1e40af,#3b82f6)">
<div class="kpi-icon">🏦</div>
<div class="kpi-title">Capital pendiente</div>
<div class="value">{fmt_money(cap_pendiente)}</div>
</div>

<div class="kpi-card" style="background:linear-gradient(135deg,#7c3aed,#a78bfa)">
<div class="kpi-icon">📈</div>
<div class="kpi-title">Interés total</div>
<div class="value">{fmt_money(int_total)}</div>
</div>

<div class="kpi-card" style="background:linear-gradient(135deg,#059669,#10b981)">
<div class="kpi-icon">✅</div>
<div class="kpi-title">Interés cobrado</div>
<div class="value">{fmt_money(int_cobrado)}</div>
</div>

<div class="kpi-card" style="background:linear-gradient(135deg,#d97706,#f59e0b)">
<div class="kpi-icon">⏳</div>
<div class="kpi-title">Interés pendiente</div>
<div class="value">{fmt_money(int_pendiente)}</div>
</div>

<div class="kpi-card" style="background:linear-gradient(135deg,#b91c1c,#ef4444)">
<div class="kpi-icon">📊</div>
<div class="kpi-title">Total por cobrar</div>
<div class="value">{fmt_money(total_por_cobrar)}</div>
</div>

</div>

<!-- 2️⃣ MÉTRICAS DIARIAS -->
<div class="dash-section-title">📅 Métricas Diarias</div>
<div class="kpi-grid">
...
<!-- 3️⃣ OPERACIONES -->
<div class="dash-section-title">📋 Operaciones</div>
<div class="kpi-grid">
...
<div class="kpi-card" style="background:linear-gradient(135deg,#0ea5e9,#06b6d4);color:white">
<div class="kpi-icon">➕</div>
<div class="kpi-title">Nuevo préstamo</div>
<div style="font-size:12px;opacity:.9">Evaluación crédito</div>
</div>
</a>
...
<a href="/bank/client-scores" class="kpi-link">
<div class="kpi-card" style="background:linear-gradient(135deg,#6366f1,#8b5cf6);color:white">
<div class="kpi-icon">⭐</div>
<div class="kpi-title">Score de clientes</div>
</div>
</a>
...
<a href="/prestamos/pagados" class="kpi-link">
<div class="kpi-card" style="background:#27ae60;cursor:pointer">
<div class="kpi-icon">📄</div>
<div class="kpi-title">Préstamos Pagados</div>
</div>
</a>
...
<!-- 4️⃣ CONTROL -->
<div class="dash-section-title">✔ Control</div>
<div class="kpi-grid">
...
def new_user():
    if request.method == "POST":
        ...
        org_id = session.get("org_id")
        if not org_id:
            flash("Sin organizacion asignada.", "danger")
            return redirect(url_for("new_user"))
        ...
        try:
            cur.execute("""
                INSERT INTO users (username, password_hash, role, phone, organization_id, created_at)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (username, pwd, role, phone, org_id, datetime.utcnow()))
            conn.commit()
...
def reassign_clients():
    ...
    cur.execute("SELECT id, username FROM users WHERE role = 'cobrador' AND organization_id = %s", (org_id,))
    ...
    if request.method == "POST":
        ...
        cur.execute("""
            UPDATE clients SET created_by = %s
            WHERE created_by = %s AND organization_id = %s;
        """, (to_id, from_id, org_id))

        cur.execute("""
            UPDATE loans SET created_by = %s
            WHERE created_by = %s AND organization_id = %s;
        """, (to_id, from_id, org_id))
...
def clients():
    ...
    if user["role"] != "cobrador":
        cur.execute("SELECT id, username FROM users WHERE role = 'cobrador' AND organization_id = %s ORDER BY username", (org_id,))
        lenders = cur.fetchall() or []

    selected_lender = request.args.get("lender_id", "all")

    if user["role"] == "cobrador" and not is_cartera_admin(user):
        cur.execute("SELECT * FROM clients WHERE created_by = %s AND organization_id = %s ORDER BY id DESC", (user["id"], org_id))
    else:
        if selected_lender == "all" or is_cartera_admin(user):
            cur.execute("SELECT * FROM clients WHERE organization_id = %s ORDER BY id DESC", (org_id,))
        else:
            cur.execute("SELECT * FROM clients WHERE created_by = %s AND organization_id = %s ORDER BY id DESC", (selected_lender, org_id))
...
def edit_client(client_id):
    ...
    org_id = session.get("org_id")
    if not org_id:
        flash("Sin organización asignada.", "danger")
        return redirect(url_for("index"))
    ...
    if not org_id:
        ...
        return redirect(url_for("index"))
    cur.execute("SELECT * FROM clients WHERE id=%s AND organization_id=%s", (client_id, org_id))
    client = cur.fetchone()
...
def new_client():
    ...
    if request.method == "POST":
        ...
        cur.execute("""
            INSERT INTO clients
            (first_name, last_name, document_id, phone, address, route, created_by, organization_id)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s)
        """, (first, last, document_id, phone, address, route, user["id"], org_id))
        conn.commit()
        cur.close()
        conn.close()
...
def client_detail(client_id):
    ...
    # ===============================
    # SCORE Y CRÉDITO RECOMENDADO
    # ===============================
    score_data = calc_client_score(conn, client_id, org_id)
    max_credito = calc_max_credito(
        score_data["prestamos_pagados"],
        score_data["score"]
    )
    score_html = f"""
    <div class="card" style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:1px solid #86efac;">
      <h3>📊 Score de crédito</h3>
      <p><b>Score:</b> {score_data['score']} &nbsp; <b>Nivel:</b> {score_data['nivel']}</p>
      <p><b>Préstamos pagados:</b> {score_data['prestamos_pagados']} &nbsp;
         <b>Atrasos:</b> {score_data['atrasos']}</p>
      <p style="font-size:18px;font-weight:700;color:#166534;">
        💰 Crédito recomendado: {fmt_money(max_credito)}
      </p>
    </div>
    """
...
def delete_loan(loan_id):
    ...
    try:
        ...
        # ============================
        # OBTENER PRESTAMO (SaaS isolation filter)
        # ============================
        cur.execute("SELECT amount, created_by FROM loans WHERE id=%s AND organization_id = %s", (loan_id, org_id))
        loan = cur.fetchone()
        ...
        # ============================
        # BORRAR PRÉSTAMO (SaaS isolation filter)
        # ============================
        cur.execute("DELETE FROM loans WHERE id=%s AND organization_id=%s", (loan_id, org_id))

        # ============================
        # DEVOLVER CAPITAL AL BANCO (SaaS: insertar organization_id)
        # ============================
        if loan_amount > 0:
            cur.execute("""
                INSERT INTO cash_reports
                (user_id, movement_type, amount, note, created_at, organization_id)
                VALUES (%s,'reverso_prestamo',%s,%s,NOW(),%s)
            """, (
                created_by,
                loan_amount,
        ...
        if discount > 0:
            cur.execute("""
                INSERT INTO cash_reports
                (user_id, movement_type, amount, note, created_at, organization_id)
                VALUES (%s,'reverso_descuento',%s,%s,NOW(),%s)
            """, (
                created_by,
                -discount,
        ...
        if total_pagado > 0:
            cur.execute("""
                INSERT INTO cash_reports
                (user_id, movement_type, amount, note, created_at, organization_id)
                VALUES (%s,'reverso_pago',%s,%s,NOW(),%s)
            """, (
                created_by,
                -total_pagado,
...
def delete_client(client_id):
    ...
    try:
        ...
        # SaaS isolation filter: verificar que el cliente pertenezca a la org
        cur.execute("SELECT id FROM clients WHERE id=%s AND organization_id=%s", (client_id, org_id))
        if not cur.fetchone():
            flash("Cliente no encontrado o no pertenece a su organización.", "danger")
            cur.close()
            conn.close()
            return redirect(url_for("clients"))

        # ============================
        # OBTENER PRÉSTAMOS DEL CLIENTE (SaaS isolation filter)
        # ============================
        cur.execute("""
        ...
        # ============================
        # BORRAR CLIENTE (SaaS isolation filter)
        # ============================
        cur.execute("DELETE FROM clients WHERE id=%s AND organization_id=%s", (client_id, org_id))
...
def loans():
    ...
    # ===============================
    # QUERY PRINCIPAL (SEGÚN ROL) - SaaS isolation: siempre filtrar por organization_id
    # ===============================
    if user["role"] == "cobrador" and not is_cartera_admin(user):
        cur.execute("""
       ...
       c.first_name, c.last_name
            ...
            WHERE l.organization_id = %s AND l.created_by = %s
            ORDER BY l.id DESC
            LIMIT 100
        """, (org_id, user["id"]))
        rows = cur.fetchall() or []
       ...
       c.first_name, c.last_name
        ...
        if filter_user:
                cur.execute("""
                    ...
                    FROM users u
                    LEFT JOIN loans l ON l.created_by = u.id AND l.organization_id = u.organization_id
                ...
                cur.execute("""
                    ...
                    FROM users u
                    LEFT JOIN loans l ON l.created_by = u.id AND l.organization_id = u.organization_id
                    WHERE u.role = 'cobrador' AND u.organization_id = %s
                    GROUP BY u.username
                    ORDER BY u.username
                """, (org_id,))
...
def new_loan():
    ...
    try:
        ...
        # =====================================================
        # 🏦 SALDO BANCO ACTUAL (SaaS isolation filter)
        # =====================================================
        if org_id:
            cur.execute("SELECT COALESCE(SUM(amount),0) AS total FROM cash_reports WHERE organization_id = %s", (org_id,))
        else:
            cur.execute("SELECT COALESCE(SUM(amount),0) AS total FROM cash_reports")
        bank_balance = float(cur.fetchone()["total"] or 0)

        # =====================================================
        # CLIENTES (multi-tenant: filtrar por org_id)
        # =====================================================
        if user.get("role") == "admin" or is_cartera_admin(user):
            if org_id:
                cur.execute("""
                    SELECT id, first_name, last_name FROM clients
                    WHERE organization_id = %s ORDER BY first_name
                """, (org_id,))
            else:
        ...
        if request.method == "POST":
            ...
            force_create = request.form.get("force_create") == "1"  # Admin override cuando denied
            amount_raw = request.form.get("amount")
            amount = float(amount_raw) if amount_raw and str(amount_raw).strip() else None
            ...
            if not client_id or amount is None or not start_str:
                flash("Todos los campos son obligatorios (cliente, monto, fecha inicio)", "danger")
            ...
            # ===============================
            # 📋 LOAN DECISION ENGINE - Evaluación automática
            # ===============================
            score_data = calc_client_score(conn, client_id, org_id)
            cur.execute("""
                SELECT COUNT(*) AS c FROM loans
                WHERE client_id = %s AND UPPER(status) = 'ACTIVO' AND organization_id = %s
            """, (client_id, org_id))
            prestamos_activos = int((cur.fetchone() or {}).get("c") or 0)
            eval_result = evaluate_loan(score_data["score"], prestamos_activos)

            # Si denied y admin NO forzó: mostrar advertencia y pedir confirmación
            if eval_result["status"] == "denied" and not force_create:
                form_data = {
                    "client_id": client_id,
                    "amount": amount,
                    "rate": rate,
                    "freq": freq or "semanal",
                    "start_str": start_str,
                ...
                flash("⚠️ Cliente no recomendado para préstamo (score < 40). Si desea continuar, marque la casilla.", "warning")
                # Saltar al renderizado del formulario con bloque de evaluación
                goto_form_with_eval = True
            else:
                goto_form_with_eval = False

            if not goto_form_with_eval:
                # ===============================
                # CÁLCULOS
                # ===============================
                ...
                # Validar organización antes de crear préstamo
                if not org_id:
                    flash("Sin organización asignada. No se puede crear el préstamo.", "danger")
                    return redirect(url_for("loans"))

                # =====================================================
                # 💰 1️⃣ RESTAR DINERO DEL BANCO (ENTREGA PRÉSTAMO) (SaaS: organization_id)
                # =====================================================
                cur.execute("""
                INSERT INTO cash_reports (
                    user_id,
                ...
                VALUES (%s,'prestamo_entregado',%s,%s,NOW(),%s)
                """, (
                    user["id"],
                    -abs(amount),
                ...
                cur.execute("""
                    ...
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'ACTIVO',%s,%s,%s,%s)
                    RETURNING id
                """, (
                    client_id,
                    amount,
                ...
                if discount_amount > 0:
                    ...
                    cur.execute("""
                        ...
                        VALUES (%s,'descuento_inicial',%s,%s,NOW(),%s)
                    """, (
                        collector_id,
                        discount_amount,
        ...
        # =====================================================
        # FORMULARIO (GET o POST con denied - mostrar evaluación)
        # =====================================================
        # Si GET con ?client_id=X: calcular evaluación para mostrar bloque.
        # Si POST denied: form_data ya fue definido arriba en el bloque POST (no sobrescribir).
        form_data = locals().get("form_data") if request.method == "POST" else None
        if request.method == "GET":
            pre_client_id = request.args.get("client_id", type=int)
            if pre_client_id:
                score_data = calc_client_score(conn, pre_client_id, org_id)
                cur.execute("""
                    SELECT COUNT(*) AS c FROM loans
                    WHERE client_id = %s AND UPPER(status) = 'ACTIVO' AND organization_id = %s
                """, (pre_client_id, org_id))
                prestamos_activos = int((cur.fetchone() or {}).get("c") or 0)
                eval_result = evaluate_loan(score_data["score"], prestamos_activos)
                form_data = {
        ...
        # Bloque de evaluación de crédito (Loan Decision Engine)
        if form_data and form_data.get("eval_result"):
            ev = form_data["eval_result"]
            sd = form_data.get("score_data", {})
            eval_bg = "#dcfce7" if ev["status"] == "approved" else "#fef3c7" if ev["status"] == "warning" else "#fee2e2"
            eval_html = f"""
            <div class="card" style="background:{eval_bg};border-left:4px solid {"#16a34a" if ev["status"]=="approved" else "#f59e0b" if ev["status"]=="warning" else "#dc2626"};margin-bottom:20px;">
              <h3>📋 Evaluación de crédito</h3>
              <p><b>Score:</b> {sd.get('score', '-')} &nbsp; <b>Nivel:</b> {sd.get('nivel', '-')}</p>
              <p><b>Monto recomendado:</b> {fmt_money(ev['max_amount'])}</p>
              <p><b>Estado:</b> {ev['status_label']}</p>
            </div>
            """
            force_checkbox = ""
            if ev["status"] == "denied" and user.get("role") == "admin":
                force_checkbox = """
                <div style="background:#fee2e2;padding:12px;border-radius:8px;margin-bottom:16px;">
                  <label><input type="checkbox" name="force_create" value="1"> Continuar de todas formas (solo admin)</label>
                </div>
                """
        else:
            eval_html = '<div class="card" style="background:#f1f5f9;margin-bottom:20px;"><p>Seleccione un cliente (o venga desde la ficha del cliente) para ver la evaluación de crédito.</p></div>'
            force_checkbox = ""

        # Valores pre-rellenados cuando hay form_data (POST denied o GET con client_id)
        sel_client = form_data.get("client_id") if form_data else None
        options = "".join([
            f"<option value='{c['id']}'{' selected' if sel_client and c['id']==sel_client else ''}>{c['first_name']} {c['last_name']}</option>"
            for c in clients
        ])
        val_amount = form_data.get("amount", "") if form_data else ""
        val_rate = form_data.get("rate", 0) if form_data else 0
        val_upfront = form_data.get("upfront_percent", 0) if form_data else 0
        val_start = form_data.get("start_str", date.today()) if form_data else date.today()
        val_term = form_data.get("term_count", 1) if form_data else 1
        val_freq = form_data.get("freq", "semanal") if form_data else "semanal"
        ...
        <div class="card">
            ...
            <form method="post">
                ...
                <label>Cliente</label>
                <select name="client_id" onchange="if(this.value)location.href='/loans/new?client_id='+this.value">
                    <option value="">-- Seleccionar --</option>
                    {options}
                </select>

                <label>Monto</label>
                <input type="number" step="0.01" name="amount" value="{val_amount}" required>

                <label>Interés %</label>
                <input type="number" step="0.01" name="rate" value="{val_rate}">

                <label>Descuento inicial (%)</label>
                <input type="number" step="0.01" name="upfront_percent" value="{val_upfront}">

                <label>Frecuencia</label>
                <select name="frequency">
                    <option value="diario" {"selected" if val_freq=="diario" else ""}>Diario</option>
                    <option value="semanal" {"selected" if val_freq=="semanal" else ""}>Semanal</option>
                    <option value="quincenal" {"selected" if val_freq=="quincenal" else ""}>Quincenal</option>
                    <option value="mensual" {"selected" if val_freq=="mensual" else ""}>Mensual</option>
                </select>

                <label>Fecha inicio</label>
                <input type="date" name="start_date" value="{val_start}">

                <label>Cuotas</label>
                <input type="number" name="term_count" value="{val_term}">
        ...
        import traceback
        print("ERROR CREANDO PRESTAMO:", e)
        traceback.print_exc()
        if conn:
            conn.rollback()
...
def loan_detail(loan_id):
    ...
    # ===============================
    # PRÉSTAMO (SaaS: filtrar por organization_id)
    # ===============================
    if org_id:
        cur.execute("""
...
def print_payment(payment_id):
    ...
    try:
        ...
        if not org_id:
            return "Sin organización asignada", 403

        conn = get_conn()
        cur = conn.cursor(cursor_factory=RealDictCursor)

        # Buscar datos de organización
        org = None
        if org_id:
            try:
                cur.execute("""
                    SELECT name, COALESCE(phone,'') AS phone, COALESCE(address,'') AS address
                    FROM organizations
                    WHERE id = %s
                """, (org_id,))
                org = cur.fetchone()
            except Exception as e:
                print("ERROR print_payment org query:", e)

        # Valores seguros para organización
        if org and org.get("name"):
            org_name = str(org.get("name", "")).strip().upper() or "EMPRESA"
            org_phone = str(org.get("phone") or "").strip()
            org_address = str(org.get("address") or "").strip()
        else:
            org_name = "EMPRESA"
            org_phone = ""
            org_address = ""

        # Validar pago (SaaS: filtrar por organization_id vía loan)
        pay_sql = """
            SELECT
                p.amount,
                p.date,
                COALESCE(p.cuota_numero, 0) AS cuota_numero,
                COALESCE(l.term_count, 1) AS term_count,
                COALESCE(l.amount, 0) AS capital_aprobado,
            ...
            WHERE p.id = %s
        """
        pay_params = [payment_id]
        if org_id:
            pay_sql += " AND l.organization_id = %s"
            pay_params.append(org_id)

        try:
            cur.execute(pay_sql, pay_params)
        except Exception as e:
            print("ERROR print_payment payment query:", e)
            cur.close()
            conn.close()
            return "Error al consultar pago", 500
        ...
        if fecha is not None:
            try:
                ...
                try:
                    fecha_txt = str(fecha)[:19] if fecha else "N/A"
                except Exception:
                    pass

        # Cálculos (evitar división por cero)
        capital = float(p.get("capital_aprobado") or 0)
        rate = float(p.get("rate") or 0)
        term = int(p.get("term_count") or 1)
        if term <= 0:
            term = 1
        pago = float(p.get("amount") or 0)
        ...
        cuota = round(total / term, 2) if term else 0
        interes_pago = round(interes_total / term, 2) if term else 0
        capital_pago = round(pago - interes_pago, 2)

        cuota_num = p.get("cuota_numero") or 0
        term_count = p.get("term_count") or 0
        cuotas_txt = f"{cuota_num}/{term_count}"

        first = p.get("first_name") or ""
        last = p.get("last_name") or ""
        client_phone = p.get("phone") or "-"
        cobrador = p.get("collector_name") or "N/A"
        tel_cobrador = p.get("collector_phone") or "-"

        # Encabezado y pie: JDM Cash Now mantiene diseño fijo, resto usa datos dinámicos
        is_jdm = (org_name == "JDM CASH NOW")
        if is_jdm:
            header_html = """<div class="center bold big">JDM CASH NOW</div>
<div class="center">LA FACTORIA DEL POZO</div>
<div class="center">TEL EMPRESA: 8495700819</div>"""
            footer_html = """<div class="center bold">GRACIAS POR SU PAGO</div>
<div class="center">AMEN</div>
<div class="center">JDM Cash Now</div>"""
        else:
            header_address = f'<div class="center">{org_address}</div>' if org_address else ''
            header_phone = f'<div class="center">TEL EMPRESA: {org_phone}</div>' if org_phone else ''
            header_html = f"""<div class="center bold big">{org_name}</div>
{header_address}
{header_phone}"""
            footer_html = f"""<div class="center bold">GRACIAS POR SU PAGO</div>
<div class="center">{org_name}</div>"""

        # HTML impresora térmica (58mm/80mm) + PDF/navegador
        html = f"""
<!DOCTYPE html>
<html>
<head>
...
@media (min-width: 400px) {{
  body {{ width: 320px; font-size: 15px; }}
}}
.ticket {{ width: 100%; }}
...
<div><b>Cliente:</b> {first} {last}</div>
<div><b>Teléfono:</b> {client_phone}</div>
...
"""
        ...
        try:
            ...
            pass
        return "Error generando recibo", 500
...
def new_payment(loan_id):
    ...
    # ===============================
    # OBTENER PRÉSTAMO (SaaS isolation filter)
    # ===============================
    user_filter = "AND l.created_by = %s" if user.get("role") == "cobrador" and not is_cartera_admin(user) else ""
    if user_filter:
        cur.execute(f"""
            SELECT l.* FROM loans l
            WHERE l.id=%s AND l.organization_id = %s AND l.created_by = %s
        """, (loan_id, org_id, user["id"]))
    else:
        cur.execute("SELECT * FROM loans WHERE id=%s AND organization_id = %s", (loan_id, org_id))
    loan = cur.fetchone()
...
def bank_legal_list():
    ...
    # ===============================
    # OBTENER COBRADORES (SaaS: solo de esta org)
    # ===============================
    collectors = []
    if user["role"] == "admin" and org_id:
        cur.execute("""
            SELECT id, username
            FROM users
            WHERE role = 'cobrador' AND organization_id = %s
            ORDER BY username
        """, (org_id,))
        collectors = cur.fetchall()

    # ===============================
    # CONSULTA DE DOCUMENTOS (SaaS: organization_id)
    # ===============================
    if user["role"] == "cobrador":
        if org_id:
            cur.execute("""
                ...
                FROM loans l
                JOIN clients c ON c.id = l.client_id
                WHERE l.created_by = %s AND l.organization_id = %s
                ORDER BY l.id DESC
            """, (user["id"], org_id))
        else:
            cur.execute("""
...
def view_legal_document(loan_id):
    ...
    # 🔐 Permisos cobrador (SaaS: validar org)
    if user["role"] == "cobrador":
        if org_id:
            cur.execute("""
                SELECT id FROM loans
                WHERE id = %s AND created_by = %s AND organization_id = %s
            """, (loan_id, user["id"], org_id))
        else:
            cur.execute("""
                SELECT id FROM loans
...
def bank_advance():
    ...
    if org_id:
        loan_filter_sql = "AND l.organization_id = %s"
...
def reportes():
    ...
    # ===============================
    # COBRADORES DISPONIBLES (SaaS: solo de esta org)
    # ===============================
    cur.execute("""
        SELECT DISTINCT p.collector_name
        FROM payments p
        JOIN loans l ON l.id = p.loan_id
        WHERE p.collector_name IS NOT NULL AND l.organization_id = %s
        ORDER BY p.collector_name
    """, (org_id,))
    cobradores = [r["collector_name"] for r in cur.fetchall()]

    if desde and hasta:

        params = [desde, hasta, org_id]
        filtro_cobrador = ""
        if cobrador:
        ...
        if tipo == "interes":
            query = f"""
                ...
                FROM payments p
                JOIN loans l ON l.id = p.loan_id
                WHERE p.date >= %s AND p.date < (%s::date + INTERVAL '1 day')
                  AND p.interest > 0 AND l.organization_id = %s
                  {filtro_cobrador}
                ORDER BY p.date
            """
        elif tipo == "capital":
            query = f"""
                ...
                JOIN loans l ON l.id = p.loan_id
                WHERE p.date >= %s AND p.date < (%s::date + INTERVAL '1 day')
                  AND p.capital > 0 AND l.organization_id = %s
                  {filtro_cobrador}
                ORDER BY p.date
            """
        else:
...
def reportes_cobradores():
    ...
    if desde and hasta:
        cur.execute("""
            SELECT
                p.collector_name AS cobrador,
                COUNT(*) AS pagos,
                SUM(p.amount) AS total_cobrado,
                SUM(p.capital) AS total_capital,
                SUM(p.interest) AS total_interes
            FROM payments p
            JOIN loans l ON l.id = p.loan_id
            WHERE p.date >= %s
              AND p.date < (%s::date + INTERVAL '1 day')
              AND p.collector_name IS NOT NULL
              AND l.organization_id = %s
            GROUP BY p.collector_name
...
def prestamos_pagados():
    ...
    if org_id:
        loan_filter_sql = "AND l.organization_id = %s"
        pay_filter_sql  = "AND l.organization_id = %s"
        params.append(org_id)
    if user["role"] == "cobrador":
        loan_filter_sql += " AND l.created_by = %s"
    ...
    # ===============================
    # 📄 LISTA DE PRÉSTAMOS PAGADOS (SaaS: organization_id)
    # ===============================
    cur.execute(f"""
        SELECT
            l.id,
...
def bank_daily_list():
    ...
    # ===============================
    # FILTRO POR COBRADOR (SaaS: org_id)
    # ===============================
    params = []
    user_filter = ""
    filter_html = ""
    ...
    if org_id:
        user_filter = "AND l.organization_id = %s"
        params.append(org_id)
    if user["role"] == "cobrador":
            ...
            if cur.fetchone():
                ...
                if org_id:
                    cur.execute("""
                        ...
                        FROM loans
                        WHERE id=%s AND status ILIKE 'activo' AND organization_id = %s
                    """, (loan_id, org_id))
                else:
                    cur.execute("""
                        SELECT remaining, amount, rate, term_count
                        FROM loans
                        WHERE id=%s AND status ILIKE 'activo'
                    """, (loan_id,))
                loan = cur.fetchone()
...
def bank_expenses():
    ...
    """)
    cur.execute("ALTER TABLE route_expenses ADD COLUMN IF NOT EXISTS organization_id INTEGER;")
    conn.commit()
    ...
    if request.method == "POST":
        ...
        if not ruta or not expense_type or not amount or amount <= 0:
            ...
            # 1️⃣ Guardar en route_expenses (historial) - SaaS: organization_id
            cur.execute("""
                INSERT INTO route_expenses (route, expense_type, amount, note, organization_id)
                VALUES (%s,%s,%s,%s,%s)
            """, (ruta, expense_type, amount, nota, org_id))

            # 2️⃣ Guardar en cash_reports (ACTA) - SaaS: organization_id
            cur.execute("""
                INSERT INTO cash_reports (
                    user_id,
                    movement_type,
                ...
                VALUES (%s, 'gasto_ruta', %s, %s, %s, NOW(), %s)
            """, (
                user["id"],
                -abs(amount),
                ruta,
    ...
    # -------------------------------
    # LISTAR GASTOS (GET) - SaaS isolation filter
    # -------------------------------
    cur.execute("""
...
def delete_route_expense(expense_id):
    ...
    # 1️⃣ Obtener el gasto (SaaS isolation filter)
    cur.execute("""
    ...
    # 2️⃣ Borrar de route_expenses (SaaS isolation filter)
    cur.execute("""
        DELETE FROM route_expenses
        WHERE id = %s AND (organization_id = %s OR organization_id IS NULL)
    """, (expense_id, org_id))

    # 3️⃣ Borrar SOLO UN gasto correspondiente en cash_reports (POSTGRES SAFE + SaaS filter)
    cur.execute("""
...
def edit_expense(expense_id):
    ...
    cur.execute("SELECT * FROM route_expenses WHERE id=%s AND (organization_id = %s OR organization_id IS NULL)", (expense_id, org_id))
    exp = cur.fetchone()
    ...
    if request.method == "POST":
        ...
        cur.execute("""
            ...
            WHERE id=%s AND (organization_id = %s OR organization_id IS NULL)
        """, (expense_type, amount, note, expense_id, org_id))
        conn.commit()
...
def bank_routes_history():
    ...
    # ===============================
    # 👤 LISTA DE COBRADORES (SaaS: solo de esta org)
    # ===============================
    cur.execute("""
        SELECT id, username
        FROM users
    ...
    # ===============================
    # 📊 CONSULTA PRINCIPAL (SaaS: WHERE l.organization_id = %s)
    # ===============================
    cur.execute(f"""
        SELECT
        ...
        WHERE UPPER(l.status) = 'ACTIVO' AND l.organization_id = %s
        {user_filter_sql}
...
def bank_delivery():
    ...
    # ===============================
    # OBTENER COBRADORES (SaaS isolation filter)
    # ===============================
    if org_id:
        cur.execute("""
            SELECT id, username
    ...
    if request.method == "POST":
        ...
        if not user_id or movement_type not in ("entrega", "devolucion") or not amount or amount <= 0:
            ...
            flash("Super admin: seleccione una organización para registrar movimientos.", "danger")
        else:
            try:
                cur.execute("""
                    INSERT INTO cash_reports (user_id, movement_type, amount, note, created_at, organization_id)
                    VALUES (%s,%s,%s,%s,NOW(),%s)
                """, (user_id, movement_type, amount, note, org_id))
                conn.commit()
                flash("Movimiento registrado correctamente.", "success")
            except Exception as e:
                conn.rollback()
                print("ERROR SQL bank_delivery INSERT:", str(e))
    ...
    # ===============================
    # HISTORIAL (SaaS isolation filter)
    # ===============================
    if filter_user:
        if org_id:
            cur.execute("""
                ...
                WHERE cr.user_id=%s AND (cr.organization_id = %s OR cr.organization_id IS NULL)
        ...
        if org_id:
            cur.execute("""
                ...
                FROM cash_reports cr
                LEFT JOIN users u ON u.id=cr.user_id
                WHERE cr.organization_id = %s OR cr.organization_id IS NULL
                ORDER BY cr.created_at DESC
            """, (org_id,))
        else:
            cur.execute("""
    ...
    # ===============================
    # BALANCE DEL DÍA (SaaS: filtrar por organization_id)
    # ===============================
    if filter_user:
        if org_id:
            cur.execute("""
        ...
        if org_id:
            ...
            """)
    b = cur.fetchone()
    total_entregado = float((b or {}).get("entregado") or 0)
    total_devuelto = float((b or {}).get("devuelto") or 0)
    balance_dia = total_devuelto - total_entregado

    # ===============================
    # TOTAL PRESTADO (SaaS isolation filter)
    # ===============================
    if filter_user:
        if org_id:
            cur.execute(
                "SELECT COALESCE(SUM(amount),0) AS t FROM loans WHERE created_by=%s AND organization_id = %s",
                (filter_user, org_id)
            )
        else:
            cur.execute(
        ...
        if org_id:
            cur.execute("SELECT COALESCE(SUM(amount),0) AS t FROM loans WHERE organization_id = %s", (org_id,))
        else:
            cur.execute("SELECT COALESCE(SUM(amount),0) AS t FROM loans")

    total_prestado = float((cur.fetchone() or {}).get("t") or 0)

    # ===============================
    # CAPITAL E INTERÉS
    # ===============================
    # (SaaS isolation filter)
    if filter_user:
        if org_id:
            cur.execute("""
                SELECT
    ...
    cap = cur.fetchone()
    capital_original = float((cap or {}).get("capital_original") or 0)
    capital_pendiente = float((cap or {}).get("capital_pendiente") or 0)
    ...
    interes_total = float((cur.fetchone() or {}).get("interes_total") or 0)
    ...
    total_pagado = float((cur.fetchone() or {}).get("total_pagado") or 0)
...
def bank_delivery_edit(delivery_id):
        ...
        if not amount or amount <= 0:
            ...
            if org_id:
                cur.execute("""
                    ...
                    SET amount=%s, note=%s
                    WHERE id=%s AND (organization_id = %s OR organization_id IS NULL)
                """, (amount, note, delivery_id, org_id))
            else:
                cur.execute("""
                    UPDATE cash_reports
                    SET amount=%s, note=%s
    ...
    # SaaS isolation filter
    if org_id:
        cur.execute("SELECT * FROM cash_reports WHERE id=%s AND (organization_id = %s OR organization_id IS NULL)", (delivery_id, org_id))
    else:
...
def bank_delivery_delete(delivery_id):
    ...
    # SaaS isolation filter
    if org_id:
        cur.execute("DELETE FROM cash_reports WHERE id=%s AND (organization_id = %s OR organization_id IS NULL)", (delivery_id, org_id))
    else:
...
def bank_routes_list():
    ...
    # ===============================
    # 👤 LISTA DE PRESTAMISTAS (SaaS isolation filter)
    # ===============================
    cur.execute("""
        SELECT id, username
        FROM users
    ...
    # ===============================
    # 📊 TOTAL EN LA CALLE POR RUTA (SaaS isolation filter)
    # ===============================
    cur.execute(f"""
        SELECT
        ...
        FROM loans l
        JOIN users u   ON u.id = l.created_by AND u.organization_id = l.organization_id
...
def bank_late():
    ...
    org_filter = " AND l.organization_id = %s" if org_id else ""
    late_params = [org_id] if org_id else []
    ...
    cur.execute(f"""
        ...
        JOIN clients c ON c.id = l.client_id
        LEFT JOIN payments p ON p.loan_id = l.id
        WHERE l.status = 'ACTIVO'{org_filter}
        GROUP BY
...
def bank_ranking():
    ...
    org_filter = " AND l.organization_id = %s" if org_id else ""
    rank_params = [org_id] if org_id else []
...
def credit_history():
    ...
    cur.execute("""
        ...
        JOIN clients c ON c.id = l.client_id
        LEFT JOIN payments p ON p.loan_id = l.id
        WHERE l.status = 'ACTIVO' AND l.organization_id = %s
        GROUP BY
            l.id, c.first_name, c.last_name, c.phone,
            l.start_date, l.frequency, l.term_count, l.remaining
    """, (org_id,))
...
# ============================================================
# 📊 SCORE DE CLIENTES - Lista con score, nivel y crédito recomendado
# ============================================================
...
def client_scores():
    ...
    user_filter = "AND c.created_by = %s" if user.get("role") == "cobrador" else ""
    params = [org_id]
    ...
    cur.execute(f"""
        ...
        FROM clients c
        WHERE c.organization_id = %s {user_filter}
    ...
    rows = ""
    for c in clients:
        sd = calc_client_score(conn, c["id"], org_id)
        max_cred = calc_max_credito(sd["prestamos_pagados"], sd["score"])
        color = "#16a34a" if sd["score"] >= 80 else "#22c55e" if sd["score"] >= 60 else "#f59e0b" if sd["score"] >= 40 else "#dc2626"
        rows += f"""
        <tr>
            <td>{c['first_name']} {c['last_name']}</td>
            <td>{c.get('phone') or '-'}</td>
            <td style="font-weight:bold;color:{color}">{sd['score']}</td>
            <td>{sd['nivel']}</td>
            <td>{sd['prestamos_pagados']}</td>
            <td>{sd['atrasos']}</td>
            <td>{fmt_money(max_cred)}</td>
            <td><a href="/clients/{c['id']}" class="btn btn-secondary">Ver</a></td>
            <td><a href="/loans/new?client_id={c['id']}" class="btn btn-primary">Nuevo préstamo</a></td>
        </tr>
        """

    if not rows:
        rows = "<tr><td colspan='9'>Sin clientes</td></tr>"

    body = f"""
    <div class="card">
        <h2>📊 Score de clientes</h2>
        <p>Lista de clientes con score, nivel y crédito recomendado.</p>
        <table>
            <thead>
                <tr>
                    <th>Cliente</th>
...
def check_client():
    ...
    if request.method == "POST":
        ...
        if not org_id:
            flash("Sin organización asignada.", "danger")
            return redirect(url_for("check_client"))

        conn = get_conn()
        cur = conn.cursor()

        # buscar cliente por documento (SaaS isolation filter)
        cur.execute("""
        ...
        if not client:
            ...
            # buscar préstamos activos del cliente (SaaS isolation filter)
            cur.execute("""
                SELECT start_date, frequency, term_count,
                       COUNT(p.id) AS pagos
                FROM loans l
                LEFT JOIN payments p ON p.loan_id = l.id
                WHERE l.client_id = %s AND l.status='ACTIVO' AND l.organization_id = %s
                GROUP BY l.id
            """, (client_id, org_id))
...
def collector_map():
    ...
    # Cobradores de la misma organización
    if org_id:
        cur.execute("""
    ...
    # Filtrar por cobrador si hay varios
    filtro_org = "AND u.organization_id = %s" if org_id else ""
    params = [org_id] if org_id else []
    if cobrador_id:
        params.append(cobrador_id)
        filtro_user = "AND cl.user_id = %s"
    else:
        filtro_user = ""
        if cobradores and len(cobradores) == 1:
            cobrador_id = cobradores[0]["id"]
            params.append(cobrador_id)
            filtro_user = "AND cl.user_id = %s"

    if not org_id:
        body = "<p>No se puede mostrar la ubicación.</p>"
        cur.close()
        conn.close()
        return render_template_string(TPL_LAYOUT, body=body, user=user, app_brand=APP_BRAND, theme=get_theme())

    cur.execute(f"""
        SELECT cl.latitude, cl.longitude, cl.updated_at, u.username
        FROM collector_location cl
        JOIN users u ON u.id = cl.user_id
        WHERE 1=1 {filtro_org} {filtro_user}
    ...
    if not loc:
        ...
        if len(cobradores) > 1:
            opts = "".join([
                f'<option value="{c["id"]}"{" selected" if c["id"]==cobrador_id else ""}>{c["username"]}</option>'
                for c in cobradores
            ])
            selector_html = f'''
            <p><form method="get" style="margin:10px 0;">
                <label>Cobrador:</label>
                <select name="cobrador_id" onchange="this.form.submit()">
                    <option value="">-- Seleccionar --</option>
                    {opts}
                </select>
            </form></p>
        ...
        {selector_html}
        <p>Sin ubicación reciente. El cobrador debe tener la app abierta para enviar su posición.</p>
        """
        return render_template_string(
            TPL_LAYOUT, body=body, user=user, app_brand=APP_BRAND, theme=get_theme(),
            flashes=get_flashed_messages(with_categories=True),
            admin_whatsapp=ADMIN_WHATSAPP
        )
    ...
    cobrador_nombre = loc.get("username", "Cobrador")
    ...
    <h2>📍 Ubicación cobrador</h2>
    {selector_html}
    <p><b>{cobrador_nombre}</b> — actualizado: {updated}</p>
...
def cobro_sabado():
    ...
    # 🔄 GENERAR ATRASOS AUTOMÁTICO (SaaS: con org_id)
    generar_atrasos(org_id)
    ...
    # =====================================================
    # FILTRO ORG Y COBRADOR (SaaS)
    # =====================================================
    filtro_loans = ""
    params_hoy = [proximo_sabado]
    if org_id:
        filtro_loans = "AND organization_id = %s"
        params_hoy.append(org_id)
    if user["role"] == "cobrador":
        filtro_loans += " AND created_by = %s"
        params_hoy.append(user["id"])

    # =====================================================
    # 💰 PRÉSTAMOS DEL SÁBADO (SaaS: organization_id)
    # =====================================================
    cur.execute(f"""
    ...
    # =====================================================
    # ⚠️ ATRASADOS (SaaS: filtrar por organization_id)
    # =====================================================
    params_atrasados = []
    filtro_atrasado = ""
    if org_id:
        filtro_atrasado = "AND l.organization_id = %s"
        params_atrasados.append(org_id)
    if user["role"] == "cobrador":
...
def bank_resumen():
    ...
    # ============================
    # 💰 TOTAL PRESTADO (SaaS: organization_id)
    # ============================
    if org_id:
        cur.execute("""
            SELECT COALESCE(SUM(amount),0) AS total
    ...
    total_prestado = float((cur.fetchone() or {}).get("total") or 0)

    # ============================
    # 💵 TOTAL COBRADO (SaaS: vía loans)
    # ============================
    if org_id:
        cur.execute("""
            ...
            FROM payments p
            JOIN loans l ON l.id = p.loan_id
            WHERE p.status <> 'ANULADO' AND l.organization_id = %s
        """, (org_id,))
    else:
        cur.execute("""
            SELECT COALESCE(SUM(amount),0) AS total
        ...
        """)
    total_cobrado = float((cur.fetchone() or {}).get("total") or 0)
    ...
    if org_id:
        cur.execute("""
            SELECT COALESCE(SUM(p.interest),0) AS total
            FROM payments p
            JOIN loans l ON l.id = p.loan_id
            WHERE p.status <> 'ANULADO' AND l.organization_id = %s
        """, (org_id,))
        ...
        cur.execute("""
            ...
            WHERE status <> 'ANULADO'
        """)
    interes_ganado = float((cur.fetchone() or {}).get("total") or 0)

    # ============================
    # 📤 DINERO GASTADO (SaaS: organization_id)
    # ============================
    if org_id:
        cur.execute("""
            SELECT COALESCE(SUM(amount),0) AS total
        ...
        cur.execute("SELECT COALESCE(SUM(amount),0) AS total FROM route_expenses")
    dinero_gastado = float((cur.fetchone() or {}).get("total") or 0)

    # ============================
    # 🏦 BANCO REAL (SaaS: cash_reports)
    # ============================
    if org_id:
        cur.execute("""
            SELECT COALESCE(SUM(amount),0) AS total
        ...
        cur.execute("SELECT COALESCE(SUM(amount),0) AS total FROM cash_reports")
    dinero_banco = float((cur.fetchone() or {}).get("total") or 0)
...
def cierre_semanal():
    ...
    try:
        ...
        cur = conn.cursor(cursor_factory=RealDictCursor)
        # ===============================
        # PERIODO DESDE ÚLTIMO CIERRE (SaaS: filtrar por org)
        # ===============================
        cur.execute("""
            SELECT week_end
        ...
        # ===============================
        # PRÉSTAMOS ENTREGADOS (SaaS: organization_id)
        # ===============================
        cur.execute("""
            SELECT l.amount, c.first_name, c.last_name
            FROM loans l
            JOIN clients c ON c.id=l.client_id
            WHERE l.start_date BETWEEN %s AND %s AND l.organization_id = %s
            ORDER BY l.start_date DESC
        """, (inicio, fin, org_id))
        prestamos = cur.fetchall()

        # ===============================
        # GASTOS RUTA (SaaS: organization_id)
        # ===============================
        cur.execute("""
            SELECT amount, note
            FROM route_expenses
        ...
        # ===============================
        # DESCUENTOS (SaaS: filtrar por loans de la org)
        # ===============================
        try:
            cur.execute("""
                SELECT amount
                FROM initial_discounts id
                JOIN loans l ON l.id = id.loan_id
                WHERE DATE(id.created_at) BETWEEN %s AND %s AND l.organization_id = %s
            """, (inicio, fin, org_id))
            descuentos = cur.fetchall()
        except Exception as e:
            print("ERROR SQL cierre-semanal descuentos:", str(e))
            descuentos = []

    except Exception as e:
        print("ERROR SQL cierre-semanal:", str(e))
        flash("Error al cargar cierre semanal. Ver consola del servidor.", "danger")
        return redirect(url_for("index"))
    finally:
        if cur:
            try:
...
def cerrar_semana():
    ...
    try:
        ...
        # ===============================
        # TOTAL PAGOS (SaaS: filtrar por loans de la org)
        # ===============================
        cur.execute("""
        ...
        # ===============================
        # TOTAL PRESTAMOS (SaaS: organization_id)
        # ===============================
        cur.execute("""
            SELECT COALESCE(SUM(amount),0) total
            FROM loans
            WHERE start_date BETWEEN %s AND %s AND organization_id = %s
        """, (inicio, fin, org_id))
        total_prestamos = float(cur.fetchone()["total"])

        # ===============================
        # TOTAL GASTOS (SaaS: organization_id)
        # ===============================
        cur.execute("""
            SELECT COALESCE(SUM(amount),0) total
            FROM route_expenses
        ...
        # ===============================
        # GUARDAR CIERRE (SaaS: organization_id)
        # ===============================
        cur.execute("""
            INSERT INTO weekly_closures
            (week_start, week_end, total_collected,
             total_loans, total_expenses, balance, closed_by, organization_id)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s)
        """, (
            inicio,
            fin,
...
def historial_cierres():
    ...
    try:
        ...
        # Super admin sin org_id: ver todos. Usuarios normales: filtrar por organization_id
        if org_id:
            cur.execute("""
                SELECT *
                FROM bank_closures
        ...
        cierres = cur.fetchall() or []

    except Exception as e:
        print("ERROR historial-cierres:", str(e))
        flash("Error al cargar historial: " + str(e), "danger")
    finally:
        if cur:
            try:
                cur.close()
    ...
    for c in cierres:
        ...
        if user and user.get("role") == "admin":
            cid = c.get("id", "")
            boton_borrar = f"""
            <form method="POST" action="/bank/borrar-cierre/{cid}"
        ...
        # Soportar columnas bank_closures y weekly_closures
        semana = c.get("closure_date") or c.get("week_start")
        sem_fin = c.get("week_end")
        if sem_fin and semana:
            sem_display = f"{semana} → {sem_fin}"
        else:
            sem_display = str(semana or "-")
        cobrado = float(c.get("total_cobrado") or c.get("total_collected") or 0)
        gastos_val = float(c.get("gastos") or c.get("total_expenses") or 0)
        balance_val = float(c.get("balance") or 0)
        closed_by = c.get("closed_by") or c.get("user_id") or "-"
        created = c.get("created_at") or c.get("closure_date") or "-"

        filas += f"""
        <tr>
            <td>{sem_display}</td>
            <td>{fmt_money(cobrado)}</td>
            <td>{fmt_money(gastos_val)}</td>
            <td>{fmt_money(balance_val)}</td>
            <td>{closed_by}</td>
            <td>{created}</td>
            <td>{boton_borrar}</td>
        </tr>
    ...
    msg_vacio = "<tr><td colspan=7 style='text-align:center;padding:20px'>Sin cierres registrados</td></tr>"
    body = f"""
    <div class="card">
...
def pagar_prestamo(loan_id):
    ...
    try:
        # OBTENER MONTO (SaaS: filtrar por organization_id)
        cur.execute("""
...
def borrar_cierre(cierre_id):
    ...
    try:
        ...
        # SaaS: solo borrar cierres de la org del usuario
        cur.execute("""
            DELETE FROM bank_closures
            WHERE id=%s AND (organization_id = %s OR organization_id IS NULL)
        """, (cierre_id, org_id))
...
def historial_depositos():
    ...
    cur.execute("""
        ...
        FROM cash_reports cr
        LEFT JOIN users u ON u.id = cr.user_id
        WHERE cr.movement_type='deposito_admin' AND cr.organization_id = %s
        ORDER BY cr.created_at DESC
        LIMIT 100
    """, (org_id,))
...
def admin_force_create():
    """Solo emergencia: recrear admin. Requiere PIN o eliminar en producción."""
    conn = get_conn()
    cur = conn.cursor()

    cur.execute("SELECT id FROM organizations WHERE slug='principal' LIMIT 1;")
    row = cur.fetchone()
    org_id = (row[0] if isinstance(row, (tuple, list)) else row.get("id")) if row else 1

    cur.execute("DELETE FROM users WHERE role != 'admin' OR username = 'admin';")
    cur.execute("""
    ...
    return "✅ Admin recreado. Usuario: admin | Contraseña: admin"
...
def delete_advance(payment_id):
    ...
    try:
        # obtener info del pago (SaaS: validar que el préstamo pertenezca a la org)
        if org_id:
            cur.execute("""
                SELECT p.loan_id, p.weeks_advanced
                FROM payments p
                JOIN loans l ON l.id = p.loan_id
                WHERE p.id=%s AND l.organization_id = %s
            """, (payment_id, org_id))
        else:
            cur.execute("""
...
def _startup():
    """Inicializar BD y migraciones (Render + local)."""
    try:
        init_db()
        from schema_migrations import run_all_migrations
        run_all_migrations(get_conn)
    except Exception as e:
        print("WARN Startup:", str(e))

# Ejecutar al cargar (Gunicorn/Render)
_startup()

if __name__ == "__main__":
    print("[JDM Cash Now] Iniciando…")
